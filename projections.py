...
class FastConnectionFieldProjection(Projection):
    ...
    def __init__(self, name, source, target, strength, delay, radius, initial_connection_kernel):
        ...
        initial_connection_kernel : ndarray
                 The initial connection kernel. 
        """
        Projection.__init__(self, name, source, target, strength, delay)
        self.radius = radius

        mask = imagen.Disk(xdensity=source.density, ydensity=source.density, bounds=BoundingBox(radius=source.size / 2),
                           size=self.radius * 2, smoothing=0)
    ...
    def activate(self):
        ...
        self.activity = self.strength * numpy.reshape(numpy.dot(self.cfs, sa),
                                                      (self.target.unit_diameter, self.target.unit_diameter))

    def get_cf(self, pos_x, pos_y):
        ...
        print(xi, xa, yi, ya)
        return numpy.reshape(self.cfs[pos_x * self.target.unit_diameter + pos_y, :],
                             (self.source.unit_diameter, self.source.unit_diameter))[xi:xa, yi:ya]

    def apply_hebbian_learning_step(self, learning_rate):
        """
...
class ConvolutionalProjection(Projection):
    ...
    def __init__(self, name, source, target, strength, delay, connection_kernel):
        ...
        assert numpy.shape(connection_kernel)[
                   0] % 2 == 1, "ERROR: initial kernel for ConvolutionalProjection has to have odd radius"
        # lets calculate edge correction factors:
        self.rad = int((numpy.shape(connection_kernel)[0] - 1) / 2)
        dim = self.target.unit_diameter
        cfs = [[self.connection_kernel[max(0, self.rad - j):2 * self.rad + 1 - max(0, (j + 1 + self.rad) - dim),
                max(0, self.rad - i):2 * self.rad + 1 - max(0, (i + 1 + self.rad) - dim)] for i in range(dim)] for j in
               range(dim)]
        self.corr_factors = numpy.array(
            [[numpy.sum(numpy.abs(connection_kernel)) / numpy.sum(numpy.abs(b)) for b in a] for a in cfs])

    def activate(self):
        """
        This returns a matrix of the same size as the target sheet, which corresponds
        to the contributions from this projections to the individual neurons.
        """
        resp = signal.fftconvolve(self.source.get_activity(self.delay), self.connection_kernel, mode='same')
        assert numpy.shape(resp) == (self.target.unit_diameter,
                                     self.target.unit_diameter), \
            "ERROR: The size of calculated projection response is " \
            + str(numpy.shape(resp)) + "units, while the size of target sheet is " \
            + str((self.target.unit_diameter, self.target.unit_diameter)) + " units"
